...
for panelIndex = 1:nPanels
    ...
    elseif isnumeric(panelFaces)
        if isvector(panelFaces)
            panelFaces = panelFaces(:).';
        end
        % Numeric rows may use NaN padding; cell faces contain only node indices.
        panelFaces = num2cell(panelFaces,2);
        panelFaces = cellfun(@(face) face(~isnan(face)),panelFaces,'UniformOutput',false);
